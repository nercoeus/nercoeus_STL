#ifndef _NER_STL_VECTOR_H_
#define _NER_STL_VECTOR_H_
...
void vector<T, Alloc>::swap(vector<T, Alloc> &v)
{
	if (this != &v)
	{
		NER_STL::swap(start, v.start);
		NER_STL::swap(finish, v.finish);
		NER_STL::swap(end_of_storage, v.end_of_storage);
	}
}
//�Ǹ����Ͳ���
template <class T, class Alloc>
...
void vector<T, Alloc>::insert_aux(iterator position, const T &x)
{
	if (finish != end_of_storage)
	{
		...
		++finish;
		NER_STL::copy_backward(position, finish - 2, finish - 1);
		*position = x;
	}
	else
	{
